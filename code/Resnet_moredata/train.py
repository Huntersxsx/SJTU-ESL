...
train_loader, validation_loader, training_batches, validation_batches = load_data('/workspace/statProject/Data/MoreData/train', '/workspace/statProject/Data/MoreData/validation/', BATCH_SIZE, get_transforms())
model = ResNet18().to(device)
criterion = nn.CrossEntropyLoss()
optimizer = optim.Adam(model.parameters(), lr=LEARNING_RATE)

logging.basicConfig(level=logging.INFO,
                    filename='/workspace/statProject/Resnet_moredata/train_state.log',
                    filemode='a',
                    format='%(message)s'
                    )
