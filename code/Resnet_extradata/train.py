...
train_loader, validation_loader, training_batches, validation_batches = load_data('/workspace/statProject/Data/RawData_extra/train', '/workspace/statProject/Data/RawData_extra/validation', BATCH_SIZE, get_transforms())
model = ResNet18().to(device)
criterion = nn.CrossEntropyLoss()
optimizer = optim.Adam(model.parameters(), lr=LEARNING_RATE)

logging.basicConfig(level=logging.INFO,
                    filename='/workspace/statProject/Resnet_extradata/train_state.log',
                    filemode='a',
                    format='%(message)s'
                    )
    ...
    for epoch in range(num_epochs):
        ...
        with torch.no_grad():
            ...
            if validation_accuracy > previous_accuracy:
                ...
                logging.info('Epoch {}: the validation_accuracy is {}'.format(epoch, validation_accuracy))
