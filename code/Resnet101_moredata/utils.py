...
def save_checkpoint(epoch, model):
    state = {'epoch': epoch,
             'model': model}
    filename = '/workspace/statProject/Resnet101/' + 'Best_checkpoint' + '.pth.tar'
    torch.save(state, filename)


def save_csvfile(my_list, name):
    df = pd.DataFrame(data=[my_list])
    df.to_csv("/workspace/statProject/Resnet101/" + name, encoding="utf-8-sig", mode="a", header=False, index=False)
